...
Design Lab follows Idea → Design → Generate → Refine → Print. A user may add one local PNG, JPEG, or WebP reference of up to 12 MB and explicitly mark the visual qualities they want to carry forward. Ollama returns editable analysis with transferable ideas and avoid-copying guidance, an editable design brief, an editable original direction, and visible editable positive and negative prompts. A reference is an inspiration-analysis input and is never silently wired into ComfyUI image conditioning.

Generation loads the application-owned `workflows/image_design_lab.api.json` graph and injects prompt, frame, seed, steps, guidance, and output prefix at runtime. The graph reuses Print Studio's FLUX.2 Klein model set. Results remain in session memory, preserve prompt/settings metadata, and keep parent relationships for subtle variations, creative variations, and natural-language refinements. New results never overwrite their parent. Prepare for Print preserves the image and provides an approximate garment-color contrast preview; background removal remains explicitly unavailable until a local workflow is configured. Send to Upscaler transfers the chosen result into the existing Image Upscaler workflow.

Ollama, vision-model, file type/size, workflow, missing-model, ComfyUI, timeout, and likely out-of-memory failures remain inline with a concrete recovery action. Reset clears only Design Lab state and is unavailable during local processing.
...
Model Studio loads dropdown vocabulary from `prompts/clothing_brand_model_prompt_components.yaml`. Presentation supports women, men, androgynous adults, girls, and boys. Girl and boy selections switch the full casting and styling vocabulary to age-appropriate ranges (4–5 through 16–17), child hair, build, pose, expression, bottomwear, and accessories; their product list contains only half-sleeve T-shirts and hoodies. Garment color and print placement remain separate choices. Accessory controls sit within the Garment group and must stay clear of the hero garment. Changing presentation automatically selects a compatible product when needed. Style changes never mutate an existing broken prompt. `Generate Broken Prompt` captures the current compatible selections as a read-only keyword brief; `Generate Final Prompt` sends that snapshot through `prompts/__prompt-expander.md` and local Ollama; the returned prompt is editable. The Image tab keeps that prompt editable and sends it to the shared Z-Image Turbo ComfyUI workflow. Re-generation preserves the final prompt and source selections. The three tabs expose the sequence Style → Prompt → Image without preventing users from inspecting later panels.
